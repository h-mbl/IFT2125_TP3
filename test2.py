    ...
    while i < nombreLigne:
        ...
        if i == 0 and courtChemin == 0:
            ...
            for j in range(nombreColonne):
                tableau[i][j] = matrice[i+1][j] + matrice[i][j]
            ...
            elementVisite.append([i,indexFirstLine])
        ...
        # sinon,
            ...
            if j-1 >= 0 and [i,j-1] not in elementVisite and i > 0:
                ...
                while tmp_i < i + 1 :
                    ...
                    if tmp_j + 1 < nombreColonne and [i,tmp_j + 1] not in elementVisite and [i,tmp_j + 1] not in gauche_position and tmp_i > 0:
                        tmp_element.append(tmp_courtChemin + matrice[i][tmp_j+1])
                        tmp_position.append([tmp_i,tmp_j+1])

                    # on verifie la gauche
                    if tmp_j - 1 >=0  and [i,tmp_j -  1] not in elementVisite and [i,tmp_j - 1] not in gauche_position and tmp_i > 0:
                        tmp_element.append(tmp_courtChemin + matrice[i][tmp_j-1])
                        tmp_position.append([tmp_i,tmp_j-1])
            ...
            if j+1 < nombreColonne  and [i,j+1] not in elementVisite and i > 0 :
                ...
                while tmp_i < i + 1:
                    ...
                    # on verifie la droite
                    if tmp_j + 1 < nombreColonne and [i,tmp_j + 1] not in elementVisite and tmp_i > 0 and [i,tmp_j + 1] not in droite_position:
                        tmp_element.append(tmp_courtChemin + matrice[i][tmp_j + 1])
                        tmp_position.append([tmp_i, tmp_j + 1])

                    # on verifie la gauche
                    if tmp_j - 1 >= 0 and [i,tmp_j - 1] not in elementVisite and tmp_i > 0 and [i,tmp_j - 1] not in droite_position:
                        tmp_element.append(tmp_courtChemin + matrice[i][tmp_j - 1])
                        tmp_position.append([tmp_i, tmp_j - 1])
